  ...
  <div class="page" v-loading="loading">
    ...
    <div class="content">
      <el-table height="100" v-if="tableData.length > 0" :data="tableData" style="width: 100%">
        <el-table-column prop="acamdyName" show-overflow-tooltip label="学院名称">
        </el-table-column>
        <el-table-column prop="majorName" show-overflow-tooltip label="专业名称">
        </el-table-column>
        <el-table-column prop="classAll" show-overflow-tooltip label="班级名称">
        </el-table-column>
  ...
  methods: {
    ...
    getData () {
      ...
      var params = {
        ...
        academyNo: this.academyName.ID,
        majorNo: this.majorName
      }
      this.$post(this.$store.state.api.classList, params).then((res) => {
        this.loading = false
        if (res.success === true) {
    ...
    toDetail (type, item) {
      if (type === 'add') {
        this.$router.push('/system/class_add')
      } else {
        this.$store.state.updateInfo = item
        this.$router.push('/system/class_detail')
      }
    },
    // 切换每页多少条
