  ...
  <div class="page" v-loading="loading">
    ...
    <div class="content">
      <el-table height="100" v-if="tableData.length > 0" :data="tableData" style="width: 100%">
        <el-table-column prop="name" show-overflow-tooltip label="楼栋">
        </el-table-column>
        <el-table-column prop="remark" show-overflow-tooltip label="类型">
        </el-table-column>
        <el-table-column prop="allRooms" show-overflow-tooltip label="房间数">
        </el-table-column>
        <el-table-column prop="remainingRooms" show-overflow-tooltip label="剩余间数">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData () {
      ...
      this.$post(this.$store.state.api.buildingList, params).then((res) => {
        if (res.success === true) {
          this.loading = false
          this.totalPage = res.data.countNum
          this.tableData = res.data.buildingList
        } else {
          this.loading = false
          console.log(res.success)
          this.$message.error(res.msg)
    ...
    toDetail (type, item) {
      if (type === 'add') {
        this.$router.push('/system/building_add?type=' + type)
      } else {
